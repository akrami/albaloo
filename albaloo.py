...
for result in results:
    wrapper += '<tr><td><a href="{0}.json">{0}</a></td><td>{1}</td><td>{2}</td><td>{3}</td></tr>'.format(result.address, result.ip, result.redirect, result.hsts)

wrapper += '</tbody></table><p id="footer">Generated by <a href="https://github.com/akrami/albaloo/">Albaloo</a></p></body></html>'
with open('{0}/index.html'.format(directory), 'w') as g:
    g.write(wrapper)
    g.close()
