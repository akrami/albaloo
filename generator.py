...
def generate_html(output, rows):
    ...
    for result in rows:
        ...
        json_result = json.loads(result.ssllab_result)
        ssllab_grade = ''
        if 'endpoints' in json_result:
            for endpoint in json_result['endpoints']:
                if 'grade' in endpoint:
                    if ssllab_grade == '':
                        ssllab_grade = endpoint['grade']
                    else:
                        ssllab_grade += ' / {0}'.format(endpoint['grade'])
        if ssllab_grade == '':
            ssllab_grade = 'Not Available'
        wrapper += '<tr><td><a href="{0}.json">{0} ↗</a></td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td></tr>'.format(
            result.address, result.ip, ssllab_grade, redirect_emoji, hsts_emoji)
